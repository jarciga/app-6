...
//Endpoint: http://IP Address:port/iot_aircon_alarm_system_backend/public/?req=vibrationAlarmHistory&deviceId=$deviceId&limit=1000
Future<List<VibrationList>> fetchVibrationListAlarmHistory(String? ipAddress, int? deviceId) async {
  final String baseUrl = 'http://$ipAddress';
  //final response = await http.get(Uri.parse('$endPoint/?req=vibrationAlarmHistory&deviceId=$deviceId&limit=1000'));
  final response = await http.get(Uri.parse('$baseUrl$endPoint/?req=vibrationAlarmHistory&deviceId=$deviceId&limit=1000'));
  final responseBody = response.body;
  return compute(parseVibrationListAlarmHistory, responseBody);
}

// A function that converts a response body into a List<Device>.
List<VibrationList> parseVibrationListAlarmHistory(String responseBody) {
...
class VibrationListAlarmHistoryScreen extends StatefulWidget {
  final String? ipAddress;
  final int? deviceIdSelected;
  final int? userId;
  const VibrationListAlarmHistoryScreen({Key? key, required this.ipAddress, required this.userId, required this.deviceIdSelected}) : super(key: key);

  @override
  State<VibrationListAlarmHistoryScreen> createState() => _VibrationListAlarmHistoryScreenState();
}

class _VibrationListAlarmHistoryScreenState extends State<VibrationListAlarmHistoryScreen> {
  late Future<List<VibrationList>> futureVibrationListAlarmHistory; // Single or one record
  ...
  */
  ...
  void initState() {
    super.initState();
    initIPAddress();
    futureVibrationListAlarmHistory = fetchVibrationListAlarmHistory(widget.ipAddress, widget.deviceIdSelected);
    _getLoginUserName();
    _getLoginUserId();
    _getLoginGroupId();
  }
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text('VIBRATION: Alarm History'),
          actions: <Widget>[
            Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                FutureBuilder<List<VibrationList>>(
                  future:
                  futureVibrationListAlarmHistory,
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      final List<VibrationList> vibrationListAlarmHistoryDataItems = snapshot.data!;
                      return Scrollbar(
                        child: SingleChildScrollView(
                          child: PaginatedDataTable(
                            source: dataSource(context, vibrationListAlarmHistoryDataItems),
...
DataTableSource dataSource(BuildContext context, List<VibrationList> vibrationListAlarmHistory) =>
    VibrationDataSource(context: context, dataList: vibrationListAlarmHistory);

class VibrationDataSource extends DataTableSource {
  final List<VibrationList> dataList;
  final BuildContext context;
  VibrationDataSource({required this.context, required this.dataList});
            ...
            child: OutlinedButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => RecommendationScreen(title: 'Vibration', recordId: vibration.recordId.toString(), recommendation: vibration.recommendation.toString()),
                  ),
                );
              },
